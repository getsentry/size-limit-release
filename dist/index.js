...
// node_modules/size-limit-action/src/SizeLimit.ts
...
var _SizeLimit = class _SizeLimit {
  formatBytes(size) {
    return import_bytes.default.format(size, { unitSeparator: " " });
  }
  formatTime(seconds) {
    if (seconds >= 1) {
      return `${Math.ceil(seconds * 10) / 10} s`;
    }
    return `${Math.ceil(seconds * 1e3)} ms`;
  }
  formatChange(base = 0, current = 0) {
    ...
    if (value > 0) {
      return `+${formatted}% \u{1F53A}`;
    }
    if (value === 0) {
      return `${formatted}%`;
    }
    return `${formatted}% \u{1F53D}`;
  }
  ...
  parseResults(output) {
    ...
    return results.reduce(
      (current, result) => {
        let time = {};
        if (result.loading !== void 0 && result.running !== void 0) {
  ...
  hasSizeChanges(base, current, threshold = 0) {
    const names = [
      .../* @__PURE__ */ new Set([...base ? Object.keys(base) : [], ...Object.keys(current)])
    ];
    const isSize = names.some(
      (name) => current[name] && current[name].total === void 0
    );
    if (!isSize) {
      return true;
    }
    return !!names.find((name) => {
      const baseResult = (base == null ? void 0 : base[name]) || EmptyResult;
  ...
  formatResults(base, current) {
    ...
    const header = isSize ? _SizeLimit.SIZE_RESULTS_HEADER : _SizeLimit.TIME_RESULTS_HEADER;
...
_SizeLimit.SIZE_RESULTS_HEADER = ["Path", "Size"];
_SizeLimit.TIME_RESULTS_HEADER = [
  "Path",
  "Size",
  "Loading time (3g)",
...
var SizeLimit = _SizeLimit;
var SizeLimit_default = SizeLimit;
...
function getArtifactsForBranchAndWorkflow(_0, _1) {
  return __async(this, arguments, function* (octokit, {
    ...
    const allWorkflows = [];
    try {
      for (var iter = __forAwait(octokit.paginate.iterator(
    ...
    try {
      ...
      )), more2, temp2, error2; more2 = !(temp2 = yield iter2.next()).done; more2 = false) {
...
// src/utils/downloadOtherWorkflowArtifact.ts
var import_exec = __toESM(require_exec());
var io = __toESM(require_io2());
var import_path = __toESM(require("path"));
function downloadOtherWorkflowArtifact(_0, _1) {
  return __async(this, arguments, function* (octokit, {
    owner,
    repo,
    artifactId,
    ...
    const artifact = yield octokit.rest.actions.downloadArtifact({
      owner,
      repo,
    ...
    const downloadFile = import_path.default.resolve(downloadPath, `${artifactName}.zip`);
    yield (0, import_exec.exec)("wget", [
      "-nv",
      "--retry-connrefused",
      "--waitretry=1",
      "--read-timeout=20",
    ...
    yield (0, import_exec.exec)("unzip", ["-q", "-d", downloadPath, downloadFile], {
      silent: true
    });
  });
}
...
// src/index.ts
var SIZE_LIMIT_HEADING = `## Bundle size \u{1F4E6} `;
var ARTIFACT_NAME = "size-limit-action";
var RESULTS_FILE = "size-limit-results.json";
function run() {
  return __async(this, null, function* () {
    var _a;
    ...
    try {
      ...
      const branchName = getInput("branch_name") || `release/${version2}`;
      const octokit = (0, import_github.getOctokit)(githubToken);
      const limit = new SizeLimit_default();
      const resultsFilePath = import_path2.default.resolve(__dirname, RESULTS_FILE);
      const release = yield octokit.rest.repos.getReleaseByTag(__spreadProps(__spreadValues({}, repo), {
        tag: version2
      }));
      if ((_a = release.data.body) == null ? void 0 : _a.includes(SIZE_LIMIT_HEADING)) {
      ...
      const sizeLimitResults = JSON.parse(
        yield import_fs.promises.readFile(resultsFilePath, { encoding: "utf8" })
      );
      const body = [
        release.data.body,
        SIZE_LIMIT_HEADING,
        ...
        markdownTable(limit.formatResults(void 0, sizeLimitResults)).replace(
          / \(added\)/gim,
          ""
        )
      ].join("\r\n\r\n");
      yield octokit.rest.repos.updateRelease(__spreadProps(__spreadValues({}, repo), {
